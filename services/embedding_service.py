...
def _draw_embedding_preview_frame(
    ...
    *,
    ...
    """Return a UI preview frame for embedding capture/extraction.

    This replaces the old server-side cv2.imshow viewer.  The frontend reads
    latest_frames through /embeddings/preview(.jpg), so operators see the same
    detected frames in the browser.
    """
    ...
    # Header strip
    header_h = 58
    cv2.rectangle(out, (0, 0), (w, header_h), (0, 0, 0), -1)
    title = f"Embedding capture | cam {int(camera_id)} | {str(member_name or 'member')} | frame {int(frame_idx)}"
    cv2.putText(out, title[:120], (12, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (0, 229, 255), 2, cv2.LINE_AA)
    sub = f"persons {len(det_boxes)} | faces {len(faces) if faces_checked else 'skip'}"
    if message:
        sub += f" | {message}"
    cv2.putText(out, sub[:120], (12, 49), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (230, 237, 243), 1, cv2.LINE_AA)
    ...
    if not det_boxes:
        cv2.putText(out, "Waiting for person detection...", (12, min(h - 24, header_h + 32)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 180, 255), 2, cv2.LINE_AA)
    return out
...
def embedding_preview_mjpeg_generator(camera_id: Optional[int] = None, max_fps: Optional[float] = None, jpeg_quality: Optional[int] = None):
    fps = float(max_fps or EMBEDDING_PREVIEW_FPS or 8.0)
...
def start_extraction(member_id: int, camera_ids: Optional[Any] = None, show_viewer: bool = True) -> dict:
    ...
    # Browser UI preview replaces server-side cv2.imshow.  Keep the old viewer
    # available only when explicitly enabled for local debugging.
    if bool(show_viewer) and EMBEDDING_ENABLE_CV2_VIEWER and (viewer_thread is None or not viewer_thread.is_alive()):
