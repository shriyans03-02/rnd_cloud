...
#     ap.add_argument("--reader", choices=["adaptive", "pyav"], default="adaptive", help="Frame reader backend. adaptive=legacy OpenCV VideoCapture; pyav=PyAV/FFmpeg low-latency reader for multi-camera scale.")
#     ap.add_argument("--pyav-thread-type", default="AUTO", help="PyAV codec thread_type (AUTO, FRAME, SLICE, NONE). AUTO is best for throughput.")
#     ap.add_argument("--pyav-drop-corrupt", action=argparse.BooleanOptionalAction, default=True, help="Tell FFmpeg/PyAV to discard corrupt packets where possible.")
...
try:
    ...
    pass
try:
    import av  # PyAV / FFmpeg reader for scalable multi-camera ingest
    PYAV_OK = True
except Exception:
    av = None
    PYAV_OK = False
...
class PyAVQueueStream:
    """Low-latency PyAV/FFmpeg frame reader.

    This is a drop-in alternative to AdaptiveQueueStream for live RTSP streams.
    It keeps decode in a dedicated worker thread and feeds only the latest frames
    to the AI pipeline.  It is especially useful after the NVDEC restream stage,
    where inputs are already low-resolution H264 ai/cam<ID> streams.

    Notes:
    - PyAV still returns CPU numpy frames because the rest of this pipeline expects
      BGR numpy arrays.  The performance win is from avoiding OpenCV VideoCapture's
      blocking/buffering behavior and from using FFmpeg options directly.
    - If PyAV is not installed, the service falls back to AdaptiveQueueStream.
    """
    ...
    def __init__(
        ...
        reconnect_log_interval: float = 2.0,
        raw_store: Optional["RenderedFrame"] = None,
        pyav_thread_type: str = "AUTO",
        pyav_drop_corrupt: bool = True,
    ):
        if not PYAV_OK:
            raise RuntimeError("PyAV is not installed. Install with: pip install av")
        self.src = str(src)
        self.src_use = str(src)
        ...
        self.reconnect_log_interval = float(max(0.0, float(reconnect_log_interval or 0.0)))
        self.raw_store = raw_store
        self.pyav_thread_type = str(pyav_thread_type or "AUTO").upper()
        self.pyav_drop_corrupt = bool(pyav_drop_corrupt)
        self.q: queue.Queue = queue.Queue(maxsize=self.queue_size)
        self.stop_flag = threading.Event()
        self.dropped = 0
        self.read_dropped = 0
        ...
        self._decoded_since_stats = 0
        self._open_capture(initial=True)
        self.thread: Optional[threading.Thread] = None
        if not self.is_file_source:
            self.thread = threading.Thread(target=self._loop, name=f"pyav-reader-{id(self)}", daemon=True)
            self.thread.start()
    ...
    def _av_options(self) -> Dict[str, str]:
        opts: Dict[str, str] = {}
        if str(self.src).lower().startswith("rtsp"):
            opts.update({
                "rtsp_transport": self.rtsp_transport,
                "fflags": "nobuffer+discardcorrupt" if self.pyav_drop_corrupt else "nobuffer",
                "flags": "low_delay",
                "max_delay": "0",
            ...
            # FFmpeg expects microseconds for these options.
            if self.open_timeout_ms > 0:
                opts["stimeout"] = str(int(self.open_timeout_ms) * 1000)
            if self.read_timeout_ms > 0:
                opts["rw_timeout"] = str(int(self.read_timeout_ms) * 1000)
        return opts

    def _open_container_once(self):
        timeout_s = None
        try:
            timeout_ms = max(int(self.open_timeout_ms or 0), int(self.read_timeout_ms or 0))
            timeout_s = max(0.1, timeout_ms / 1000.0) if timeout_ms > 0 else None
        except Exception:
            timeout_s = None
        try:
            if timeout_s is not None:
                return av.open(self.src_use, mode="r", options=self._av_options(), timeout=timeout_s)
            return av.open(self.src_use, mode="r", options=self._av_options())
        except TypeError:
            return av.open(self.src_use, mode="r", options=self._av_options())

    def _open_capture(self, initial: bool = False) -> bool:
        try:
            container = self._open_container_once()
            try:
                if container.streams.video:
                    st = container.streams.video[0]
                    try:
                        st.thread_type = self.pyav_thread_type
                    except Exception:
                        pass
                    try:
                        fps = float(st.average_rate) if st.average_rate is not None else 0.0
                        self._source_fps = fps if math.isfinite(fps) and fps > 0.0 else 0.0
                    except Exception:
                        self._source_fps = 0.0
            except Exception:
            ...
            if initial:
                self._log(f"[PYAV] cannot open source initially: {self.src} ({e})")
            return False
        old = None
    ...
    def _maybe_reconnect(self, reason: str) -> None:
        ...
        if ok:
            self._reconnect_failures = 0
            self._next_reconnect_mono = 0.0
            self._log(f"[PYAV] reconnected: {self.src} (reason={reason})")
        else:
            self._reconnect_failures += 1
            delay = self._backoff_delay(self._reconnect_failures)
            self._next_reconnect_mono = time.monotonic() + delay
            self._log(f"[PYAV] reconnect failed: {self.src} (reason={reason}, failures={self._reconnect_failures}, next_retry={delay:.2f}s)")

    def _publish_frame(self, frame_bgr: np.ndarray, ts: float) -> None:
        if frame_bgr is None or getattr(frame_bgr, "size", 0) == 0:
            return
        self._last_frame_mono = time.monotonic()
        self._capture_seq += 1
        ...
        raw_store = self.raw_store
        if raw_store is not None:
            raw_meta = {"raw": True, "src": str(self.src), "capture_seq": cap_seq, "frame_seq": cap_seq, "capture_ts": float(ts), "reader": "pyav"}
            try:
                raw_store.set(frame_bgr, meta=raw_meta, ts=float(ts))
            except TypeError:
                try:
                    raw_store.set(frame_bgr, meta=raw_meta)
                except Exception:
                    pass
            except Exception:
                pass
        item = (frame_bgr, float(ts), cap_seq)
        try:
            self.q.put_nowait(item)
        except queue.Full:
            self._drop_oldest()
            try:
                ...
                self._drop_oldest()
        self._decoded_since_stats += 1
        now = time.monotonic()
        if (now - float(self._last_stats_mono)) >= 5.0:
            fps = float(self._decoded_since_stats) / max(1e-6, now - float(self._last_stats_mono))
            self._decoded_since_stats = 0
            self._last_stats_mono = now
            self._log(f"[PYAV] src={self.src} decoded_fps={fps:.1f} q={self.qsize()} dropped={self.dropped} read_dropped={self.read_dropped}")

    def _decode_loop_once(self) -> bool:
        with self._container_lock:
        ...
        try:
            for packet in container.demux(video_stream):
                ...
                for frame in frames:
                    ...
                    try:
                        ts = time.time()
                        # Convert directly to BGR so the rest of the project stays unchanged.
                        arr = frame.to_ndarray(format="bgr24")
                    except Exception:
                        continue
                    self._publish_frame(arr, ts)
            return False
        except Exception as e:
            self._log(f"[PYAV] decode error: {self.src} ({e})")
            return False

    def _loop(self):
        while not self.stop_flag.is_set():
            if not self._connected or self._container is None:
                self._maybe_reconnect(reason="disconnected")
                self.stop_flag.wait(0.02)
                continue
            ok = self._decode_loop_once()
            if self.stop_flag.is_set():
                break
            now_mono = time.monotonic()
            if not ok or (now_mono - float(self._last_frame_mono)) >= float(self.freeze_seconds):
                self._maybe_reconnect(reason=f"freeze>{self.freeze_seconds:.2f}s")
            else:
                self.stop_flag.wait(0.002)

    def _read_file_frame(self) -> Tuple[bool, Optional[np.ndarray], float]:
        # File mode uses a small one-frame demux/decode step. Most deployments use RTSP.
        with self._container_lock:
            container = self._container
        if container is None or self._eof:
            return False, None, 0.0
        try:
            video_stream = container.streams.video[0]
            for packet in container.demux(video_stream):
                for frame in packet.decode():
                    ...
                    ts = time.time()
                    self._capture_seq += 1
                    self.last_read_seq = int(self._capture_seq)
                    self._last_frame_mono = time.monotonic()
                    if self.raw_store is not None:
                        self.raw_store.set(arr, meta={"raw": True, "src": str(self.src), "reader": "pyav"}, ts=float(ts))
                    return True, arr, float(ts)
            self._eof = True
            return False, None, 0.0
        except Exception:
            self._eof = True
    ...
    def is_opened(self) -> bool:
        return bool(self._connected) and (self._container is not None)
...
def make_video_stream(args, src, queue_size, rtsp_transport, raw_store=None, use_opencv=True, **kwargs):
    """Factory for capture readers.

    --reader adaptive keeps the legacy OpenCV path.
    --reader pyav uses PyAV/FFmpeg with low-latency options and a dedicated
    decode thread per stream. If PyAV is missing, this falls back to OpenCV so
    deployments do not crash unexpectedly.
    """
    reader = str(getattr(args, "reader", "adaptive") or "adaptive").strip().lower()
    if reader == "pyav":
        if PYAV_OK:
            try:
                print(f"[INIT] PyAV reader enabled for source: {src}")
                return PyAVQueueStream(
                    src=src,
                    queue_size=queue_size,
                    rtsp_transport=rtsp_transport,
                    raw_store=raw_store,
                    pyav_thread_type=str(getattr(args, "pyav_thread_type", "AUTO") or "AUTO"),
                    pyav_drop_corrupt=bool(getattr(args, "pyav_drop_corrupt", True)),
                    **kwargs,
                )
            except Exception as e:
                print(f"[WARN] PyAV reader failed for {src}; falling back to OpenCV AdaptiveQueueStream: {e}")
        else:
            print("[WARN] --reader pyav requested but PyAV is not installed. Install with: pip install av. Falling back to OpenCV.")
    return AdaptiveQueueStream(src, queue_size=queue_size, rtsp_transport=rtsp_transport, use_opencv=use_opencv, raw_store=raw_store, **kwargs)
...
def main():
    ...
    for i, raw_src in enumerate(args.src):
        src = raw_src.strip() if isinstance(raw_src, str) else raw_src
        camera_db_id = int(args.camera_ids[i])
        vs = make_video_stream(
            args, src, queue_size=args.queue_size, rtsp_transport=args.rtsp_transport, use_opencv=True,
            freeze_seconds=float(args.stream_freeze_seconds), open_timeout_ms=int(args.stream_open_timeout_ms),
            read_timeout_ms=int(args.stream_read_timeout_ms), reconnect_base_delay=float(args.stream_reconnect_base_seconds),
            reconnect_max_delay=float(args.stream_reconnect_max_seconds), reconnect_jitter=float(args.stream_reconnect_jitter),
            reconnect_log_interval=float(args.stream_reconnect_log_interval),
...
class TrackingRunner:
    ...
    def start(self) -> None:
        ...
        for sid, raw_src in enumerate(args.src):
            ...
            self._raw_by_cam[int(camera_db_id)] = raw_buf
            vs = make_video_stream(
                args, src, queue_size=args.queue_size, rtsp_transport=args.rtsp_transport, use_opencv=True, raw_store=raw_buf,
                freeze_seconds=float(args.stream_freeze_seconds), open_timeout_ms=int(args.stream_open_timeout_ms),
                read_timeout_ms=int(args.stream_read_timeout_ms), reconnect_base_delay=float(args.stream_reconnect_base_seconds),
                reconnect_max_delay=float(args.stream_reconnect_max_seconds), reconnect_jitter=float(args.stream_reconnect_jitter),
                reconnect_log_interval=float(args.stream_reconnect_log_interval),
